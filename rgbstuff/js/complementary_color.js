...
function convertToComplementary(hexColor) {
    ...
    const nb = 255 - b;
    return {
        hex: `#${Math.floor(nr).toString(16).padStart(2, '0')}${Math.floor(ng).toString(16).padStart(2, '0')}${Math.floor(nb).toString(16).padStart(2, '0')}`,
        rgb: [nr,ng,nb]
    };
}
...
color_colorPicker.addEventListener('input', () => {
    const output = convertToComplementary(color_colorPicker.value);
